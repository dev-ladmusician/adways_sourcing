...
class User extends CORE_Controller
{
    ...
    function change_password()
    {
        ...
        if (strlen($password) == 0) {
            ...
            $rtv = $this->user_model->change_password($user_id, password_hash($password, PASSWORD_BCRYPT));

            if ($rtv) {
                $this->session->set_flashdata('message', '비밀 번호를 변경하는데 성공 했습니다.');
                redirect('user/detail?userid=' . $user_id);
            } else {
                $this->session->set_flashdata('message', '비밀번호가 변경되지 않았거나 비밀 번호를 변경하는데 오류가 발생했습니다.');
                redirect('user/detail?userid=' . $user_id);
            }
        }
    }
    ...
    function delete() {
        $notice_id = $this->input->get('userId');
        $rtv = $this->user_model->delete($notice_id);
        if (!$rtv) $this->session->set_flashdata('message', '유저를 삭제하는데 오류가 발생했습니다.');
        else $this->session->set_flashdata('message', '유저를 삭제하였습니다.');
        redirect('user/');
    }
